...
public class Diameter {
    public static class Solution {
        ...
        private static DiameterReturn diameter(BinaryTreeNode<Integer> root){
            if(root==null)
                return new DiameterReturn(0,0);
            DiameterReturn leftRes = diameter(root.left);
            DiameterReturn rightRes = diameter(root.right);
            int height = 1 + Math.max(leftRes.height, rightRes.height);
            int diameterFromChildren = Math.max(leftRes.diameter, rightRes.diameter);
            int calculatedDiameter = 1+leftRes.height+rightRes.height;
            return new DiameterReturn(Math.max(calculatedDiameter, diameterFromChildren), height);
        }
    ...
    public static void main(String[] args) throws NumberFormatException, IOException {
        ...
        int d = Solution.diameterOfBinaryTree(root);
